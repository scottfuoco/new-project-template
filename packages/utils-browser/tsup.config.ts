...
const managePackages: ESBuildPlugin = {
  ...
  setup(build) {
    ...
    build.onResolve({ filter }, (args) => {
      const isCjs =
        build.initialOptions?.define?.["TSUP_FORMAT"].includes("cjs");
      const excludeDeps = isCjs ? nonExternalDeps : nonExternalDepsWorkspace;
...
export default defineConfig({
  entry: ["src/**/*.{js,ts,tsx,jsx,mts,cts,mjs,cjs}"],
  format: ["esm", "cjs"],
  outDir: "dist",
  dts: true,
  splitting: true,
